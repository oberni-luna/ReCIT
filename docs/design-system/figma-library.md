...
| D47 | `Ranger mes livres · Light`, titre de carte vs `TextStyle` | **Manque côté design system.** Le titre de carte de la maquette est une serif d'environ 13 pt ; aucun token Alegreya n'existe à cette taille (`title50` = 18). Le code rend `footnote200Bold` — OpenSans 12 — donc la bonne graisse et la bonne taille, mais pas la bonne famille | ouverte — soit un token Alegreya 13 est ajouté, soit la maquette passe à OpenSans |
