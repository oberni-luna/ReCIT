...
| D44 | `Tri manuel · Glissement` (`135:3934` / `136:4110`), `Tri manuel · Dépôt sur étagère vide` (`137:4286`) | **Frames périmés, conservés.** Ils maquettaient le drag maison — rang soulevé en `Shadow/Pressed`, emplacement libéré en `background/disable`, bande teintée en cible. Ce drag ne prenait pas sur appareil et a été remplacé par le reorder natif de `List` : le lever, le trou et la surbrillance sont ceux du système et ne sont pas stylables. Le point ouvert sur `background/disable` = `gray/600` en sombre tombe avec eux | périmés — **à ne pas supprimer**, ils documentent une intention ; à remaquetter si le geste redevient maison |
